...
**优先级: P1**　**状态: 6 处损失中 5 处已修复, 最后 1 处(目录)于 2026-09-22 修复**
...
现在的 `assess_raw_quality()` 检查的是字符数、乱码率、重复率、块数 ——
**没有任何一项能发现「代码块全丢」或「目录混进正文」**。
原文「方案一」建议的代码围栏/占位符比例检查尚未实现。
不过在当前 `pdf_pdfplumber` 路径上，① 已不复现，所以这项的紧迫性下降了。
